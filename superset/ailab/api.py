...
class SqlLabRestApi(BaseSupersetApi):
    ...
    ## Add start - Adham - March 03, 2025: Add new RESTFul API for python code execution
    @expose("/run_python_code/", methods=("POST",))
    @protect()
    @protect()
    @statsd_metrics
    @requires_json
    ...
    def run_python_code(self) -> Response:
        """Run python code.
        ---
        post:
          summary: Run python code
          requestBody:
            description: Python code and input params
            required: true
            content:
              application/json:
                schema:
